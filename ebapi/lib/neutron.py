...
class Networks(NeutronBase):
    ...
    def getInternalNetworks(self):
        ...
        if not response.ok:
            elog.error(
                "failed fetching internal networks: %s"
    ...
    def getExternalNetworks(self):
        ...
        if not response.ok:
            elog.error(
                "failed fetching external networks: %s"
                % eutil.rcolor(response.status_code)
            )
            return None
    ...
    def createInternalNetwork(self, netName="", subnetName=""):
            ...
            "subnets": [
                {
                    ...
                    "allocation_pools": [
                        {"start": "192.168.150.2", "end": "192.168.150.254"}
                    ],
        ...
        if not response.ok:
            elog.error(
                "failed to create network: %s" % eutil.rcolor(response.status_code)
            )
        ...
        elog.info(
            "network %s created successfully: %s"
            % (eutil.bcolor(netName), eutil.bcolor(networkID))
        )
        return networkID
    ...
    def deleteInternalNetwork(self, networkID):
        ...
        if not response.ok:
            elog.error(
                "deleting network %s failed: %s"
                % (eutil.rcolor(networkID), eutil.rcolor(response.status_code))
            )
            elog.error(response.text)
            return False

        elog.info(
            "deleting network %s success: %s"
            % (eutil.bcolor(networkID), eutil.gcolor(response.status_code))
        )
        return True
    ...
    def getNetworkByName(self, networkID):
        ...
        if not response.ok:
            elog.error(
                "failed fetching network %s: %s"
                % (eutil.bcolor(networkID), eutil.rcolor(response.status_code))
            )
            return None
...
class Ports(NeutronBase):
    ...
    def getPortIDByMacAddress(self, macAddress):
        ...
        if not response.ok:
            elog.error(
                "failed to get fetch ports: %s" % eutil.rcolor(response.status_code)
            )
            return None
    ...
    def attachQoSPolicy(self, portID, policyID):
        requestURL = self.portsURL + "/" + portID
        payload = {"port": {"qos_policy_id": policyID}}

        response = self.client.put(requestURL, payload)
        if not response.ok:
            elog.error(
                "failed to attach QoS policy %s to port %s: %s"
                % (
        ...
        if policyID != qosPolicyID:
            elog.error(
                "mismatch in qos_policy_id: given = %s: set = %s"
                % (eutil.bcolor(policyID), eutil.bcolor(qosPolicyID))
            )
            return False
    ...
    def detachQoSPolicy(self, portID):
        requestURL = self.portsURL + "/" + portID
        payload = {"port": {"qos_policy_id": None}}

        response = self.client.put(requestURL, payload)
        if not response.ok:
            elog.error(
                "failed to detach QoS policy from port %s: %s"
                % (eutil.bcolor(portID), eutil.rcolor(response.status_code))
            )
            elog.error(response.text)
            return False
...
class Routers(NeutronBase):
    ...
    def getAllRouters(self):
        ...
        if not response.ok:
            elog.error(
                "failed to get all routers for project %s: %s"
                % (eutil.bcolor(self.projectID), eutil.rcolor(response.status_code))
            )
            return None
...
class QoS(NeutronBase):
    ...
    def createPolicy(self, name, shared=False):
        ...
        if not response.ok:
            elog.error(
                "failed to create QoS policy: %s" % eutil.rcolor(response.status_code)
            )
            elog.error(response.text)
            return None
    ...
    def createBandwidthLimitRules(self, policyID, maxBurst, maxBandwidth):
        ...
        if not response.ok:
            elog.error(
                "failed to create bandwidth limit rule: %s"
                % eutil.rcolor(response.status_code)
            )
            elog.error(response.text)
            return False
    ...
    def deletePolicy(self, policyID):
        ...
        if not response.ok:
            elog.error(
                "failed to delete QoS policy: %s" % eutil.rcolor(response.status_code)
            )
            return False
