...
def test_publicAccess():
    ...
    payload    = {
        "vnc_public_ip"  : vncPublicIP,
        "vnc_public_port": vncPublicPort
    }
    ...
    assert rc == 0
    dev = re.findall('dev (\S+)', out, re.M)
    assert dev
    dev = dev[0]
    rc, out = firewall.sudo('ifconfig %s' % dev)
    assert rc == 0
    privateIP = re.findall('inet (\S+)', out, re.M)
    assert privateIP
    privateIP = privateIP[0]

    # port forwarding using iptables
    rule  = 'iptables -A PREROUTING -t nat -p tcp '
    opts  = '-i %s --dport %s -j DNAT --to %s:26000' \
            % (dev, vncPublicPort, nginxIP)
    cmd   = rule + opts
    ...
    opts  = '-o %s --dport %s -d %s -j SNAT --to-source %s' \
            % (dev, vncPublicPort, nginxIP, privateIP)
    cmd   = rule + opts
    rc, _ = firewall.sudo(cmd)
    assert rc == 0
