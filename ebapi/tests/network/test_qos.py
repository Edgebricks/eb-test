...
def setup_test(request):
    ...
    if serPassword:
        ...
        iperfServer = RemoteMachine(iperfServerIP, serUserName,
                                    keyFile=serKeyFile)
    else:
    ...
    if vmPassword:
        ...
        iperfClient = RemoteMachine(iperfClientIP, vmUserName,
                                    keyFile=vmKeyFile)
    else:
    ...
    if rc != 0:
        pytest.skip('iperf3 not installed on iperfServer: %s'
                    % eutil.bcolor(iperfServerIP))

    rc, _ = iperfClient.run('which iperf3')
    if rc != 0:
        pytest.skip('iperf3 not installed on iperfClient: %s'
